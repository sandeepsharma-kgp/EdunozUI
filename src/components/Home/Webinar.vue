<template>
<b-container fluid style="background-color: #eceef8; padding: 20px;">
    <b-row align-h="center">
    <b-col align-self="center" sm>
      <b-img left :src="require('../../assets/images/blur-close-up-desk-433604.jpg')" alt="Left image" style="width: 90%; height:405px; padding:15px 40px; border-radius:45px;"></b-img>
    </b-col>
    <b-col align-self="center" sm>
      <div style="align-content: center;">
      <h4>Webinar</h4>
      <p>Lorem ipsum, or lipsum as it is sometimes known, is dummy text used in laying out print, graphic or web designs. 
        The passage is attributed to an unknown typesetter in the 15th century who is thought to have scrambled parts of 
        Cicero's De Finibus Bonorum et Malorum for use in a type specimen book.
      </p>
      <b-btn >See Details</b-btn>
      </div>
    </b-col>
    </b-row>
</b-container>
</template>

<style scoped>
.btn{
  background-color : #d66d02 !important;
  width : 50%;
}
</style>

